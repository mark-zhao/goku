...
//search vm by ip
func SearchByIP(client *gophercloud.ServiceClient, ip string)   map[string]interface{}{
	url := listDetailURL(client)
	if ip !=  ""{
		url+="?all_tenants=1&ip="
		url+=ip
	}
	var r GetResult
	...
	if len(m["servers"]) != 0 {
		//fmt.Println("名称:",m["servers"][0]["name"]," - ","ID:",m["servers"][0]["id"]," - ","状态:",m["servers"][0]["status"])
		return m["servers"][0]
	}else {
		return nil
	}
...
// GET vnc by id
func GetVnc (client *gophercloud.ServiceClient, id string) (Url string,err error) {
	b := map[string]interface{}{
	...
	var r GetResult
	if resp, err := client.Post(getURL(client, id+"/remote-consoles"), b, &r.Body, &gophercloud.RequestOpts{
		OkCodes: []int{200},
	});err != nil {
		return "",err
	}else {
		...
		bx, err := json.Marshal(r.Body)
		err = json.Unmarshal(bx, &m)
		if err != nil {
			return "", err
		}
		return m["remote_console"]["url"], nil
	}
}